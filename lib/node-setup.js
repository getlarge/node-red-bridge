...
nodeRed.start = async conf => {
  try {
    const options = {
      host: conf.aloesHost,
      port: conf.aloesPort,
      path: `${conf.aloesRestApiRoot}/Users/login`,
      method: 'POST',
...
nodeRed.init = async (app, conf) => {
  try {
    ...
    if (externalModules && Object.keys(externalModules).length > 0) {
      Object.keys(externalModules).forEach(
        key => (settings.functionGlobalContext[key] = externalModules[key]),
      );
    }
